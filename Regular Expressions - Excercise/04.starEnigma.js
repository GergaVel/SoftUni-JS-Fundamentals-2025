function starEnigma(arr) {
    ...
    let starPattern = /[star]/gi; // i==case insesnitive
    ...
    for (let currentMessage = 1; currentMessage <= messageCount; currentMessage++){
        ...
        let count = starMatches ? starMatches.length : 0; //if there is a match with length > 0, count them , otherwise give me 0 suvpadenia
    ...
    let planetPttern = /@(?<name>[A-Za-z]+)[^@\-!:>]*:(?<population>\d+)[^@\-!:>]*!(?<attackType>[AD])![^@\-!:>]*->(?<soldiers>\d+)/;
    
    let attackedPlanets = [];
    let destroyedPlanets = [];

    for (let message of decryptedMessages){
        let match = message.match(planetPttern);
